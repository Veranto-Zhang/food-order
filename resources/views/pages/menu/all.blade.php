...
<div id="TopNav" class="relative flex items-center justify-between px-4 my-[30px]">
    ...
    <div class="flex flex-col items-center">
        <h1 class="font-semibold text-lg">All Menus</h1>
        <p class="text-darkGrey text-sm">Tersedia {{ $allMenus->count() }} Menu</p>
    </div>
    <div class="dummy-btn w-12"></div>
</div>

<section id="Result" class=" relative flex flex-col gap-4 px-4">
    <div>
            <form action="{{ route('pages.menu.all') }}" method="GET" id="searchForm">
            <label
                class="flex items-center w-full rounded-xl p-[14px_20px] gap-5 bg-white focus-within:ring-1 focus-within:ring-[#ff914d] transition-all duration-300 border border-gray-200">
                <img src="{{ asset('assets/images/icons/search.svg') }}" class="w-5 h-5 flex shrink-0 opacity-40" alt="icon">
                <input type="text" name="search"
                    class="appearance-none outline-none w-full font-medium placeholder:text-gray-400 placeholder:font-normal"
